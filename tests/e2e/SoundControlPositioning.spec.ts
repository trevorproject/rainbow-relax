...
import {
  assertAllSoundTogglesVisible,
  assertToggleStateChanged,
  assertAllTogglesMuted,
  assertAllTogglesUnmuted,
  clickToggleSafely,
} from '../fixtures/assertionsHelper';
...
test.describe('Sound Control Panel Positioning', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.getByTestId('sound-control-button').waitFor({
      state: 'visible',
      timeout: 15_000,
    });
  ...
  test('should position panel above button when there is not enough space below on mobile', async ({ page }) => {
    ...
    await homePage.soundControlButton.scrollIntoViewIfNeeded();
    await expect(homePage.soundControlButton).toBeVisible({ timeout: 10_000 });

    await homePage.soundControlButton.click();
    await expect(homePage.soundPanel).toBeVisible({ timeout: 5_000 });
  });
  ...
  test.describe('Breathing Exercise Sound Control', () => {
    test('should display sound control button during intro phase', async ({ page }) => {
      ...
      await expect(exercisePage.soundControlButton).toBeVisible({ timeout: 10_000 });
    });
    ...
    test('should display sound control button during exercise phase', async ({ page }) => {
      ...
      await expect(exercisePage.timer).toBeVisible({ timeout: 25_000 });
    ...
    test('should open audio settings panel when clicked during intro', async ({ page }) => {
      ...
      await expect(exercisePage.soundPanelTitle).toBeVisible({ timeout: 5_000 });
    });
    ...
    test('should open audio settings panel when clicked during exercise', async ({ page }) => {
      ...
      await expect(exercisePage.soundControlButton).toBeAttached({ timeout: 10_000 });
      await exercisePage.soundControlButton.scrollIntoViewIfNeeded();
      await expect(exercisePage.soundControlButton).toBeVisible({ timeout: 10_000 });
  ...
  test.describe('Floating Audio Control Functionality', () => {
    test('should display floating audio control button with correct test identifiers', async ({ page }) => {
      ...
      await expect(exercisePage.soundControlContainer).toBeVisible({ timeout: 10_000 });
      await expect(exercisePage.soundControlButton).toBeVisible({ timeout: 10_000 });
    });
    ...
    test('should open panel when button is clicked', async ({ page }) => {
      ...
      await exercisePage.openSoundControlPanel();
      await expect(exercisePage.soundPanel).toBeVisible({ timeout: 5_000 });
    });
    ...
    test('should toggle background sounds setting', async ({ page }) => {
      ...
      await exercisePage.openSoundControlPanel();
      await expect(exercisePage.backgroundToggle).toBeVisible({ timeout: 5_000 });
    ...
    test('should toggle instructions setting', async ({ page }) => {
      ...
      await exercisePage.openSoundControlPanel();
      await expect(exercisePage.instructionsToggle).toBeVisible({ timeout: 5_000 });
    ...
    test('should toggle exercise guide setting', async ({ page }) => {
      ...
      await exercisePage.openSoundControlPanel();
      await expect(exercisePage.guideToggle).toBeVisible({ timeout: 5_000 });
    ...
    test('should mute/unmute all sounds with mute all button', async ({ page }) => {
      ...
      await exercisePage.openSoundControlPanel();
      await expect(exercisePage.muteAllButton).toBeVisible({ timeout: 5_000 });
    ...
    test('should close panel when clicking outside', async ({ page }) => {
      ...
      await closeSoundControlPanel(page);

      await expect(exercisePage.soundPanel).not.toBeVisible({ timeout: 5_000 });
    });
