...
export function extractWidgetConfigParams(): Record<string, string> {
  ...
  WIDGET_CONFIG_PARAMS.forEach((param) => {
    ...
    if (value !== null) configParams[param] = value;
  });
  return configParams;
}

export function buildUrlWithParams(path: string, existingParams?: Record<string, string>): string {
  const allParams = { ...extractWidgetConfigParams(), ...existingParams };
