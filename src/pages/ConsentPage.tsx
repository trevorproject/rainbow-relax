...
export const ConsentPage = () => {
  ...
  useEffect(() => {
    ...
    if (!forceConsent) {
      ...
      const isSlowByType = isSlowConnectionType(connection.effectiveType);
      const isSlowBySpeed = connection.downlink !== undefined && connection.downlink < MIN_DOWNLINK_MBPS;
    ...
    fetch(`${basePath}/app-size.json`, { signal: abortControllerRef.current.signal })
      .then((res) => {
        if (!res.ok) throw new Error("Failed to fetch app size");
        return res.json();
      })
  ...
  if (!showPrompt || !appSize) return null;

  return (
    <div data-testid="consent-page" className="min-h-screen bg-[var(--background-global)]">
      <ConsentPrompt
